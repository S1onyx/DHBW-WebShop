// backend/apis/wishlists/items/postWishlistItem.js
const postWishlistItemModel = require('../../../models/wishlists/items/postWishlistItemModel');

async function postWishlistItem(req, res, wishlistId) {
  ...
  req.on('end', async () => {
    ...
    try {
      const result = await postWishlistItemModel(wishlistId, userId, product_id, quantity);
      ...
      console.error('[POST WISHLIST ITEM ERROR]', { wishlistId, product_id, error: err });
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: false, error: 'Server error during wishlist insert', code: 500 }));
    }
  });
}
